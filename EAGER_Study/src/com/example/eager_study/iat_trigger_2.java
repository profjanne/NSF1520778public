...
public class iat_trigger_2 extends Service{
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
		...
		int iat_work = settings.getInt("IAT_work", -1);
		...
		if ((!WithinDistance(current_lat, current_lon, home_lat, home_lon))&&(iat_work==0)){
			count=0;
			...
			public void run() {
				if (count==0){
					...
					if (WithinDistance(past_lat, past_lon, current_lat, current_lon)){
		        		...
		        		notificationIntent.putExtra("prompt_type", "work");
